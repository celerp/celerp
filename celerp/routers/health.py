...
async def connector_authorize_url(
    ...
    try:
        _, ownership_created = await claim_connector_ownership(
            session,
            company_id,
            platform,
    ...
    async def _cleanup_new_claim() -> bool:
        ...
        try:
            ...
            await session.rollback()
            return False
        if isinstance(cancelled, dict) or cancelled.status_code not in (200, 404):
            await session.rollback()
            return False
        try:
            await release_connector_ownership(session, company_id, platform)
    ...
    async def _error(message: str) -> dict:
        cleaned = await _cleanup_new_claim()
        if cleaned:
            return {"error": message}
        return {
            "error": (
                f"{message} The connection could not be fully cancelled; "
                "use Disconnect before retrying."
            )
        }
    ...
    try:
        ...
        return await _error("Cannot reach relay.")
    except httpx.TimeoutException:
        return await _error("Relay timed out.")
    except Exception as exc:
        return await _error(str(exc))
    if isinstance(response, dict):
        message = str(response.get("error") or "Relay authorization failed.")
        return await _error(message)

    if response.status_code == 200:
        authorize_url = response.json().get("authorize_url", "")
        if not authorize_url:
            return await _error("Authorization URL was not returned.")
        await session.commit()
    ...
    return await _error(detail or f"Relay returned {response.status_code}.")
