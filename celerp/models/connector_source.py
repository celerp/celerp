...
class ConnectorSource(Base):
    ...
    connection is checked against the store the records came from. config_id
    is the connection the store was last confirmed for, so every new
    connection is checked once, even to the same store address."""
    ...
    config_id: Mapped[int | None] = mapped_column(sa.Integer, nullable=True)
