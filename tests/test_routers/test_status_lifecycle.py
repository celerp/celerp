...
async def test_revert_blocked_when_payments_exist(client):
    ...
    await client.post(f"/docs/{inv}/finalize", headers=_h(token))
    await client.post(f"/docs/{inv}/payment", headers=_h(token), json={"payment_date": "2026-01-15", "amount": 10})
...
async def test_void_blocked_from_partial_status(client):
    ...
    await client.post(f"/docs/{inv}/finalize", headers=_h(token))
    await client.post(f"/docs/{inv}/payment", headers=_h(token), json={"payment_date": "2026-01-15", "amount": 40})
