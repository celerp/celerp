...
async def _register(client, email: str = "admin@docs.test") -> str:
    r = await client.post("/auth/register", json={"company_name": "Docs Co", "email": email, "name": "Admin", "password": "pwvalid1"})
...
async def test_docs_validation_guards_and_edge_cases(client):
    ...
    r = await client.post(f"/docs/{inv}/payment", headers=_h(token), json={"payment_date": "2026-01-15", "amount": 1, "bank_account": "1111"})
    ...
    r = await client.post(f"/docs/{inv}/payment", headers=_h(token), json={"payment_date": "2026-01-15", "amount": 108, "bank_account": "1111"})
    assert r.status_code == 409
...
async def test_auto_je_entries_balanced_and_account_codes(client):
    ...
    await client.post(f"/docs/{inv}/payment", headers=_h(token), json={"payment_date": "2026-01-15", "amount": 107, "bank_account": "1111"})
