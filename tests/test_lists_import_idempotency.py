...
@pytest.mark.asyncio
async def test_lists_import_batch_idempotency(client, session):
        ...
        "records": [
            {
                ...
                "source": "import",
                "idempotency_key": "list-created-" + entity_id,
            }
        ]
    }

    r1 = await client.post("/lists/import/batch", headers=headers, json=payload)
    assert r1.status_code == 200, r1.text
    assert r1.json() == {"created": 1, "skipped": 0, "updated": 0, "errors": []}

    r2 = await client.post("/lists/import/batch", headers=headers, json=payload)
