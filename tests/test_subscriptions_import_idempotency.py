...
@pytest.mark.asyncio
async def test_subscriptions_import_batch_idempotency(client, session):
    ...
    entity_id = "sub-test-" + uuid.uuid4().hex[:8]
    payload = {
        "records": [
            {
                "entity_id": entity_id,
                ...
                "source": "import",
                "idempotency_key": "sub-created-" + entity_id,
            }
        ]
    }

    r1 = await client.post("/subscriptions/import/batch", headers=headers, json=payload)
    assert r1.status_code == 200, r1.text
    assert r1.json() == {"created": 1, "skipped": 0, "updated": 0, "errors": []}

    r2 = await client.post("/subscriptions/import/batch", headers=headers, json=payload)
