...
@pytest.mark.asyncio
async def test_future_doc_type_is_enforced(client, session):
    """A doc of a novel doc_type with duplicate non-splittable items -> 409.

    Proves enforcement keys on entity_type == 'doc', not a known-doc-type list.
    """
    t = await _register(client)
    cid = await _company_id(session)
