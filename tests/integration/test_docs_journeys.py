...
async def test_invoice_partial_then_full_payment_updates_balances(journey_api):
    ...
    pay1 = await journey_api.post(
        ...
        json={"payment_date": "2026-01-15", "amount": 400.0, "method": "cash", "reference": "p1", "idempotency_key": _u()},
    )
    ...
    pay2 = await journey_api.post(
        f"/docs/{inv_id}/payment",
        json={"payment_date": "2026-01-15", "amount": 600.0, "method": "bank", "reference": "p2", "idempotency_key": _u()},
    )
...
async def test_invoice_void_blocks_payment(journey_api):
    ...
    pay = await journey_api.post(
        f"/docs/{inv_id}/payment",
        json={"payment_date": "2026-01-15", "amount": 100.0, "method": "cash", "reference": "after-void", "idempotency_key": _u()},
    )
...
async def test_credit_note_reduces_original_invoice_outstanding(journey_api):
    ...
    pay = await journey_api.post(
        f"/docs/{inv_id}/payment",
        json={"payment_date": "2026-01-15", "amount": 200.0, "method": "cash", "reference": "pre-credit", "idempotency_key": _u()},
    )
