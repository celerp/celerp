...
Two harnesses, for the two shapes of migration test. `mig_db` builds one table in
its own schema, for exercising a single migration's SQL. `fresh_db` provisions a
whole throwaway database, for the tests that run the real migration path end to
end and so need alembic_version and every table alembic creates.
"""
