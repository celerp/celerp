...
@pytest.mark.asyncio
async def test_accounting_import_batch_idempotency(client, session):
    ...
    r1 = await client.post("/accounting/import/batch", headers=headers, json=payload)
    assert r1.status_code == 200, r1.text
    assert r1.json() == {"created": 1, "skipped": 0, "updated": 0, "errors": []}

    r2 = await client.post("/accounting/import/batch", headers=headers, json=payload)
