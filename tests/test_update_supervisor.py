...
def _run_supervisor(tmp_path, *, sentinel_text=None, run_update=None, reconcile=None,
                    max_sleeps=3):
    """Run `_start` with the first API process exiting; return what happened."""
    ...
    hand_over = MagicMock(side_effect=lambda s: (events.append("hand_over"), (_ for _ in ()).throw(_Handover()))[1])
...
def _result(outcome, reason=""):
    return {"ok": outcome == update.OK, "outcome": outcome, "reason": reason}
...
def test_failed_update_keeps_serving_the_current_version(tmp_path, result_outcome):
    outcome, events, _ = _run_supervisor(
        tmp_path, sentinel_text="update 1.1.0",
        run_update=lambda t, s: (_result(result_outcome, "install failed: x"), ()))
...
def test_failed_undo_stops_the_supervisor(tmp_path):
    outcome, events, _ = _run_supervisor(
        tmp_path, sentinel_text="update 1.1.0",
        run_update=lambda t, s: (_result(update.ROLLBACK_FAILED, "restore broke"), ()))
    assert outcome == 1
...
def test_interrupted_update_is_resolved_before_migrating(tmp_path):
    _pending(tmp_path)
    outcome, events, _ = _run_supervisor(
        tmp_path, reconcile=lambda s: (_result(update.ROLLED_BACK), ()))
...
def test_interrupted_update_finished_hands_over(tmp_path):
    _pending(tmp_path)
    outcome, events, _ = _run_supervisor(
        tmp_path, reconcile=lambda s: (_result(update.OK), (_Proc("api"), _Proc("ui"))))
    assert outcome == "handed_over"
    assert events == ["reconcile", "stop_children", "hand_over"]


def test_interrupted_update_that_cannot_be_undone_does_not_start(tmp_path):
    _pending(tmp_path)
    outcome, events, _ = _run_supervisor(
        tmp_path, reconcile=lambda s: (_result(update.ROLLBACK_FAILED, "x"), ()))
    assert outcome == 1
...
def test_hand_over_posix_replaces_the_process(monkeypatch):
    ...
    steps = MagicMock()
    execv = MagicMock(side_effect=_Handover())
    monkeypatch.setattr(cli.os, "name", "posix")
    monkeypatch.setattr(cli.os, "execv", execv)
    with pytest.raises(_Handover):
        cli._hand_over(steps)
    steps.stop_cluster.assert_called_once()
    execv.assert_called_once_with(cli.sys.executable,
                                  [cli.sys.executable, "-m", "celerp", "start"])
...
def test_hand_over_windows_waits_on_the_new_supervisor(monkeypatch):
    ...
    steps.stop_cluster.assert_called_once()
    cli.subprocess.call.assert_called_once_with([cli.sys.executable, "-m", "celerp", "start"])
...
def _upgrade(tmp_path, *, running=False, blockers=(), target="1.1.0", result=None):
    from click.testing import CliRunner

    from celerp.cli import main

    run_update = MagicMock(return_value=(result or _result(update.OK), ()))
    with (
        patch("celerp.cli._read_config", return_value=CFG),
        patch("celerp.cli._config_to_env", return_value={}),
        patch("celerp.cli.ensure_database"),
        patch("celerp.cli._update_steps", return_value=MagicMock()),
        patch("celerp.config.config_path", return_value=tmp_path / "config.toml"),
...
def test_upgrade_refuses_an_unfinished_update(tmp_path):
    _pending(tmp_path)
    res, run_update = _upgrade(tmp_path)
    assert res.exit_code == 1
    assert "celerp start" in res.output
    run_update.assert_not_called()
