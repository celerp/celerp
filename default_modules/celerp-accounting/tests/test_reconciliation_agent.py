...
def _seed_upload(company_id: str, content: bytes, filename: str, content_type: str) -> str:
    file_id = f"ai_up_test_{_uuid.uuid4().hex[:8]}"
    d = upload_dir()
    (d / f"{file_id}.bin").write_bytes(content)
    (d / f"{file_id}.meta").write_text(json.dumps({
        "filename": filename, "content_type": content_type, "size": len(content), "company_id": company_id,
    }))
    return file_id
...
async def test_import_file_csv_and_xlsx(client):
    ...
    csv_id = _seed_upload(company_id, _CSV, "march.csv", "text/csv")
    ...
    odd_id = _seed_upload(company_id, b"When,What,HowMuch\n2026-03-01,Fee,-5\n", "odd.csv", "text/csv")
    ...
    foreign_id = _seed_upload(other_company, _CSV, "theirs.csv", "text/csv")
    r = await _import_file(client, h, recon["id"], foreign_id)
    assert r.status_code == 422
    assert foreign_id in r.json()["detail"]

    broken_id = _seed_upload(company_id, b"not a workbook", "broken.xlsx", XLSX_CONTENT_TYPE)
...
async def test_workbench_caps_and_scopes(client):
    ...
    file_id = _seed_upload(company_id, b"Date,Description,Amount\n" + rows.encode(), "big.csv", "text/csv")
...
@pytest.mark.asyncio
async def test_line_create_rejects_unknown_account_and_repeats_cleanly(client):
    h = await _auth(client)
    company_id = await _company_id(client, h)
    bank = await _create_bank(client, h)
    recon = await _create_recon(client, h, bank["id"])
    file_id = _seed_upload(company_id, _CSV, "march.csv", "text/csv")
    ...
    session = (await client.get(f"/accounting/reconciliation/{recon['id']}", headers=h)).json()
    assert session["created_count"] == 1
    assert session["reconciled_je_ids"].count(je_id) == 1
