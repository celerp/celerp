...
async def _reg(client) -> str:
    ...
    r = await client.post("/auth/register", json={"company_name": "AccCo", "email": addr, "name": "Admin", "password": "validpass1"})
...
async def test_pnl_payment_visible_after_receiving(client):
    ...
    r = await client.post(f"/docs/{doc_id}/payment", headers=_h(tok), json={
        "payment_date": "2026-01-20", "amount": 200.0, "bank_account": "1111",
    })
    assert r.status_code == 200, r.text
...
async def test_pnl_payment_excluded_outside_date_range(client):
    ...
    r = await client.post(f"/docs/{doc_id}/payment", headers=_h(tok), json={
        "payment_date": "2025-06-10", "amount": 150.0, "bank_account": "1111",
    })
    assert r.status_code == 200, r.text
