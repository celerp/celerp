...
async def _get_fiscal_and_currency(token: str) -> tuple[str, str]:
...
def setup_routes(app):
    ...
    async def ar_aging(request: Request):
        ...
        return base_shell(
            ...
            _date_filter_bar("/reports/ar-aging", date_from, date_to, preset, settings_link="/settings?tab=terms"),
    ...
    async def ap_aging(request: Request):
        ...
        return base_shell(
            ...
            _date_filter_bar("/reports/ap-aging", date_from, date_to, preset, settings_link="/settings?tab=terms"),
    ...
    async def sales_report(request: Request):
        ...
        return base_shell(
            ...
            _date_filter_bar("/reports/sales", date_from, date_to, preset,
                             settings_link="/settings?tab=terms",
...
def _date_filter_bar(base_url: str, date_from: str, date_to: str, active_preset: str,
                     settings_link: str = "", extra_params: str = "") -> FT:
...
def _report_index() -> FT:
    reports = [
        ("/reports/ar-aging", "AR Aging", "Outstanding receivables by age bucket"),
        ("/reports/ap-aging", "AP Aging", "Outstanding payables by age bucket"),
        ("/reports/sales?group_by=customer", "Sales Report", "Revenue by customer, item, or period"),
        ("/reports/purchases?group_by=supplier", "Purchases Report", "Spending by supplier, item, or period"),
        ("/reports/expiring?days=30", "Expiring Items", "Items expiring within 30 days"),
        ("/accounting/pnl", "Profit & Loss", "Income statement for a period"),
        ("/accounting/balance-sheet", "Balance Sheet", "Assets, liabilities, and equity"),
    ]
    return Div(
        *[
            A(
