# Celerp - Cosmetics & skincare screenshots

Full set of Celerp screens captured for **Lumera Skincare** (fictional demo data). Part of the [Celerp press kit](../../README.md).

### Dashboard - revenue trend, receivables aging, and inventory mix

![Dashboard - revenue trend, receivables aging, and inventory mix](dashboard.webp)

### Inventory - attributes, barcodes, and multi-location valuation

![Inventory - attributes, barcodes, and multi-location valuation](inventory.webp)

### Manufacturing cost worksheet - BOM, labor, overhead, and rolled-up unit cost

![Manufacturing cost worksheet - BOM, labor, overhead, and rolled-up unit cost](manufacturing-worksheet.webp)

### Printable production worksheet with materials and workflow

![Printable production worksheet with materials and workflow](worksheet-print.webp)

### Invoices and sales documents

![Invoices and sales documents](invoices.webp)

### Production planning - work orders by status, priority, and due date

![Production planning - work orders by status, priority, and due date](production-planning.webp)

### Chart of accounts

![Chart of accounts](chart-of-accounts.webp)

### Trial balance that ties out to zero

![Trial balance that ties out to zero](trial-balance.webp)

### General ledger with full account drill-down

![General ledger with full account drill-down](general-ledger.webp)

### Balance sheet generated live from the event-sourced ledger

![Balance sheet generated live from the event-sourced ledger](balance-sheet.webp)

### Immutable audit log of every change

![Immutable audit log of every change](audit-log.webp)

### Role-based access permissions

![Role-based access permissions](permissions.webp)

### Built-in REST API for integrations

![Built-in REST API for integrations](rest-api.webp)

## Usage & copyright

These screenshots depict the **Celerp application UI**, which is proprietary
(see `/legal/OFFICIAL-COMPONENTS.md`; the UI layer is *not* covered by the BSL core or MIT module licenses).

- Screenshots (c) 2026 Noah Severs. All rights reserved. Provided to document and promote Celerp;
  you may use them unmodified in articles, reviews, and listings about Celerp with attribution to
  Celerp. Do not alter the interface shown or imply endorsement.
- Product imagery embedded in the screenshots is CC0 / public domain (sourced via Openverse) and
  needs no attribution.
- Company and brand names shown (Lumera Skincare, Meridian Coffee Roasters, Aurelia Atelier,
  Northbound Apparel) are fictional demo data.

For use of the Celerp name and logo, see `/legal/TRADEMARK.md`.
